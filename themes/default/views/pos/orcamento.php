...
<head>
    <meta charset="utf-8">
    <title>Orçamento <?php echo date("ymd-Hi")." | " . $Settings->site_name; ?></title>
    <base href="<?= base_url() ?>"/>
...
<button type="button" class="no-print" style="font-size: 30px; position: fixed; top: 10px; left: 10px; background: #09BB0B; border-color: #09BB0B; box-shadow: 1px 1px 20px 9px #525252; color: #FFF;" onclick="window.print();">IMPRIMIR</button>
<table align="center">
    <tr>
        <td align="center">
<div>
    <div class="bill" id="bill">
        <div id="product-list">
            <div style="width:100%;float:left;">
	             <div style="width:25%;float:left;padding-top:2px">
	             <span class="logo-lg"><?php echo '<img src="'.base_url('uploads/logo2.png').'" />'; ?></span>
	            </div>
	             ...
	             <span style="width:98%;padding:0.5%;margin:0.5%;margin-top:20px;font-size:11px;float:left;">Orçamento: <b><?php echo date("ymd-Hi");?></b></span>
	             <span style="width:98%;padding:0.5%;margin:0.5%;font-size:9px;float:left;">Emissão: <b><?php echo date("d/m/Y H:i");?></b></span>
	             <span style="width:98%;padding:0.5%;margin:0.5%;font-size:9px;float:left;">Vendedor: <b id="user1"></b></span>
	            </div>
            </div>
                    
                    <div style="width:100%;float:left;margin-top:2px;">
                    <hr>
	            <h6><b>NÃO É DOCUMENTO FISCAL - NÃO É VÁLIDO COMO RECIBO E COMO GARANTIA DE
MERCADORIA - NÃO COMPROVA PAGAMENTO</b></h6>
            
             </div>
             <div style="width:100%;float:left;margin-top:2px;">
 
		      <div style="width:98%;text-align:center:float;font-size:11px;background:#ccc;color:#000;margin:10px 0px;border:1px solid #ccc"><b>DADOS DO CLIENTE</b></div>
		      <span style="width:50%;padding:0.5%;margin:0.5%;float:left;font-size:12px;">NOME: <b id="cliente1"></b></span>
	             <span style="width:48%;padding:0.5%;margin:0.5%;float:left;font-size:12px;">ENDEREÇO: <b id="cliente2"></b> <b id="cliente3"></b>, <b id="cliente4"></b>, <b id="cliente5"></b> - <b id="cliente6"></b></span>
	             <span style="width:50%;padding:0.5%;margin:0.5%;float:left;font-size:12px;">TELEFONE: <b id="cliente7"></b></span>
	             <span style="width:48%;padding:0.5%;margin:0.5%;float:left;font-size:12px;">EMAIL: <b id="cliente8"></b></span>
             </div>
              <div style="width:100%;float:left;margin-top:10px;">
		 <div style="width:98%;text-align:center:float;font-size:11px;background:#ccc;color:#000;margin:10px 0px;border:1px solid #ccc"><b>DADOS DOS PRODUTOS/SERVIÇOS</b></div>
             </div>
            <table style="margin-bottom: 0;font-size:12px;" id="billTable" class="table table-striped table-condensed">
                <thead>
                <tr>
        ...
        <div id="totals">
            <table style="width:100%; float:right; padding:5px; color:#000; background: #FFF;" id="totalTable">
                ...
                <tr>
                    ...
                    <td style="border-right: 1px solid #ddd; text-align:right; padding-right:10px; font-weight:bold;"><span id="ts_con">_________</span></td>
                </tr>
                
                <tr>
                    <td style="border-left: 1px solid #ddd; padding-left:10px; text-align:left;font-size:14px ">Frete</td>
                    <td style="text-align:right; padding-right:10px; font-weight:bold;">
                        <span id="ds_con">__________</span></td>
                    <td style="padding-left:10px; text-align:left; font-weight:normal;font-size:14px">Forma de pagamento</td>
                    <td style="border-right: 1px solid #ddd; text-align:right; padding-right:10px; font-weight:bold;"><span id="ts_con">___________________</span></td>
                </tr>
                
                <tr>
                    <td colspan="2"
                        style="padding: 5px 0px 5px 10px; text-align:left; font-size: 1.2em; border: 1px solid #333; font-weight:bold; background:#333; color:#FFF;">
                        <?= lang('total_payable'); ?> + (Frete)
                    </td>
                    <td colspan="2"
                        style="text-align:right; padding:5px 10px 5px 0px; font-size: 1.2em; border: 1px solid #333; font-weight:bold; background:#333; color:#FFF;">
...
<script type="text/javascript">
    ...
    <?php unset($Settings->protocol, $Settings->smtp_host, $Settings->smtp_user, $Settings->smtp_pass, $Settings->smtp_port, $Settings->smtp_crypto, $Settings->mailpath, $Settings->timezone, $Settings->setting_id, $Settings->default_email, $Settings->version, $Settings->strip); ?>
...
<script type="text/javascript">
    ...
    function selecionarCliente(id){
			$.getJSON("../customers/getCostumersbyID?id=" + id, function(dados2){
				$("#cliente1").text(dados2.name);
				$("#cliente2").text(dados2.endereco);
				$("#cliente3").text(dados2.numero);
				$("#cliente4").text(dados2.bairro);
				$("#cliente5").text(dados2.cidade);
				$('#cliente6').text(dados2.estado);
				$('#cliente7').text(dados2.phone);
				$('#cliente8').text(dados2.email);

			});
	}
	
	function selecionarUser(id){
			$.getJSON("../customers/getUserbyID?id=" + id, function(dados2){
				$("#user1").text(dados2.first_name+ " " + dados2.last_name);
    ...
    */
    ...
    function loadDados(){
        var vendedor = get('spos_vendedor');
        if(vendedor!="" && vendedor!=null && vendedor!=0){
        selecionarUser(vendedor);
        }
    ...
    if (get('spositems')) {
        ...
        $.each(spositems, function () {
            ...
            var newTr = $('<tr></tr>');
            var newTr2 = '';
             if(item_comment!="" && typeof item_comment !== "undefined"){
	            newTr2 = ("<br>* " + item_comment);
            }
            tr_html = '<td>' + item_name + ' (' + item_code + ')'+ newTr2 +'</td>';
