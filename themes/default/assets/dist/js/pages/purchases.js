    $(document).ready(function() {
        ...
        $(document).on("change", '.rquantity', function () {
            ...
            var new_qty = parseFloat(formatReal($(this).val())),
        ...
        $(document).on("change", '.rcost', function () {
            var row = $(this).closest('tr');
            var new_cost =  formatReal($(this).val());
    ...
    function numberToReal(numero = 0, decimal = 2) { // formato real brasil com decimal

        return new Intl.NumberFormat('de-DE',  {minimuSignificantDigits:decimal, maximumFractionDigits:decimal} ).format(numero);
...
function loadItems() {
    ...
    if (get('spoitems')) {
        ...
        $.each(spoitems, function () {
            ...
            tr_html += '<td style="padding:2px;"><input class="form-control input-sm kb-pad text-center rquantity" name="quantity[]" type="text" value="' + numberToReal(item_qty, 3) + '" data-id="' + row_no + '" data-item="' + item_id + '" id="quantity_' + row_no + '" onClick="this.select();"></td>';
            tr_html += '<td style="padding:2px; min-width:80px;"><input class="form-control input-sm kb-pad text-center rcost" name="cost[]" type="text" value="' + formatMoney(item_cost) + '" data-id="' + row_no + '" data-item="' + item_id + '" id="cost_' + row_no + '" onClick="this.select();"></td>';
